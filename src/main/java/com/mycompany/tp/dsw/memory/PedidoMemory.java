...
public class PedidoMemory implements PedidoDao {
    ...
    public void modificarPedido(Pedido pedidoModificado) {
        ...
        Optional<Pedido> existePedido = pedidos.stream().filter(p -> 
            p.getId().equals(pedidoModificado.getId())).findFirst();
        ...
        // Ahorra tener que pasar un Pedido completo y solamente el id y los parametros a modificar
        existePedido.ifPresent(v -> {
            if (estadoModificado != null) v.setEstado(estadoModificado);
            if (itemsPedidoModificado != null) v.setItems(itemsPedidoModificado);
            });
